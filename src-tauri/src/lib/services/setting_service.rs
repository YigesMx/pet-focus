...
use super::super::models::setting::Setting;
...
impl SettingService {
    ...
    /// 设置值
    pub async fn set(db: &DatabaseConnection, key: &str, value: &str) -> Result<Setting> {
        ...
        let model = if let Some(existing_model) = existing {
            // 更新现有设置
            let mut active_model: setting::ActiveModel = existing_model.into();
    ...
    /// 获取所有设置
    pub async fn list(db: &DatabaseConnection) -> Result<Vec<Setting>> {
    ...
    /// 设置布尔值
    pub async fn set_bool(db: &DatabaseConnection, key: &str, value: bool) -> Result<Setting> {
