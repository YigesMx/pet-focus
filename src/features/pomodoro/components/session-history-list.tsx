...
interface SessionHistoryListProps {
  /** 要排除的 session ID（例如当前活动的 session） */
  excludeSessionId?: number | null
  /** 是否显示为卡片形式 */
  asCard?: boolean
}

export function SessionHistoryList({ excludeSessionId, asCard = false }: SessionHistoryListProps) {
  const { data: allSessions = [] } = useSessions(true) // 显示所有sessions，包括已归档的
  ...
  const sessions = useMemo(() => {
    if (excludeSessionId === null || excludeSessionId === undefined) {
      return allSessions
    }
    return allSessions.filter(s => s.id !== excludeSessionId)
  }, [allSessions, excludeSessionId])
    ...
    <>
      {sessions.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed py-12 text-center text-muted-foreground">
          ...
          <div className="text-base font-medium">暂无历史会话</div>
          <p className="max-w-sm text-sm">完成专注会话后将在这里显示</p>
        </div>
      ) : (
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>过往的专注会话记录</CardDescription>
        </CardHeader>
        <CardContent>
          {content}
        </CardContent>
